...
(function () {
  ...
  function initCustomCursor() {
    ...
    var interactiveEls =
      "a, button, .skill-tab, .portfolio-item, .service-card-new, .bento-card, input, textarea";
  ...
  function initScrollReveal() {
    ...
    function safeFrom(target, vars) {
      var targets = gsap.utils.toArray(target);
      if (!targets.length) return;

      var st = vars && vars.scrollTrigger;
    ...
    function safeScrollTriggerCreate(vars) {
      if (!vars) return;
    ...
    // Testimonial
    safeFrom(".testimonial-card-wrap", {
      opacity: 0,
      y: 40,
      duration: 0.8,
      ease: "power3.out",
    ...
    // Contact
    safeFrom(".contact-form-wrap, .contact-info-card ", {
      opacity: 0,
      y: 40,
      duration: 0.8,
      ease: "power3.out",
    ...
    // safeFrom("#contact .contact-info-card", {
    //   opacity: 0,
    //   x: -30,
    //   duration: 0.6,
    //   ease: "power3.out",
  ...
  function initCounters() {
    var counters = document.querySelectorAll(
      ".bento-stat .stat-num[data-target]",
    );
    ...
    function runCounters() {
      ...
      counters.forEach(function (el) {
        ...
        function step(ts) {
          ...
          el.textContent =
            Math.round(start + eased * (target - start)) + suffix;
  ...
  function initMagneticButtons() {
    document
      .querySelectorAll(".btn-primary-hero, .btn-outline-hero")
      .forEach(function (btn) {
        btn.addEventListener("mousemove", function (e) {
          ...
          btn.style.transform =
            "translate(" + x * 0.18 + "px, " + y * 0.18 + "px) translateY(0)";
        });
  ...
  function initTilt() {
    ...
    if (portfolioItems.length) {
      VanillaTilt.init(Array.from(portfolioItems), {
        max: 4,
        speed: 300,
        glare: false,
      });
